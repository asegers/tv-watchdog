...
public class TraktService {
	...
	private static final String MSG_WAIT_BEFORE_RETRY_ERROR = "Couldn't sleep 10 seconds";
	...
	final static Logger logger = LogManager.getLogger(TraktService.class);

	public ArrayList<JSONObject> getShowsDataFromTrakt(ArrayList<String> showSlugs) {
		// this is the logic for downloading all possible info for a show upon
		// first Search query to Trakt (via API)
		// add search query api here next... in place of showSlugs array.. or
		// check if showSlugs is null, then call search...? separate method?
	...
	private Show getLastEpisode(Show show) {
		String lastEpisodeUrl = TRAKT_API_URL + show.getIdSlug() + "/last_episode?extended=full";
		Response lastEpisodeData = RestAssured.given().header(HEADER_CONTENT_TYPE_KEY, HEADER_CONTENT_TYPE_VALUE)
				.header(HEADER_TRAKT_API_VERSION_KEY, HEADER_TRAKT_API_VERSION_VALUE)
				.header(HEADER_TRAKT_API_KEY_KEY, HEADER_TRAKT_API_KEY_VALUE).when().get(lastEpisodeUrl);
		...
		if (lastEpisodeData.statusCode() == 200) {
			...
			lastEpisode = LocalDateTime.parse(lastEpisodeData.jsonPath().getString(RESPONSE_EPISODE_DATE_FIELD),
					formatter);
			...
			try {
				...
				logger.error(MSG_WAIT_BEFORE_RETRY_ERROR, e);
			}
			logger.info(MSG_RETRYING);
	...
	private Show populateShowFromSlug(String slug) {
	...
	public Show populateShow(Show show) {
		show.setStatus("");
		show = getNextEpisode(show);
		show = getLastEpisode(show);
		show.setCurrentSeason(getLatestSeasonNumber(show));
	...
	public int getLatestSeasonNumber(Show show) {
		String seasonsUrl = TRAKT_API_URL + show.getIdSlug() + "/seasons";
		Response seasonsData = RestAssured.given().header(HEADER_CONTENT_TYPE_KEY, HEADER_CONTENT_TYPE_VALUE)
				.header(HEADER_TRAKT_API_VERSION_KEY, HEADER_TRAKT_API_VERSION_VALUE)
				.header(HEADER_TRAKT_API_KEY_KEY, HEADER_TRAKT_API_KEY_VALUE).when().get(seasonsUrl);
		...
		if (seasonsData.statusCode() == 200) {
			...
			for (int i = 0; i < arrayJson.length(); i++) {
				...
				if (objSeasonNumber > maxSeason)
					maxSeason = objSeasonNumber;
			}
	...
	private Show getNextEpisode(Show show) {
		String nextEpisodeUrl = TRAKT_API_URL + show.getIdSlug() + "/next_episode?extended=full";
		Response nextEpisodeData = RestAssured.given().header(HEADER_CONTENT_TYPE_KEY, HEADER_CONTENT_TYPE_VALUE)
				.header(HEADER_TRAKT_API_VERSION_KEY, HEADER_TRAKT_API_VERSION_VALUE)
				.header(HEADER_TRAKT_API_KEY_KEY, HEADER_TRAKT_API_KEY_VALUE).when().get(nextEpisodeUrl);
		...
		if (nextEpisodeData.statusCode() == 200) {
			...
			DateTimeFormatter formatter = DateTimeFormatter.ofPattern(INCOMING_DATE_FORMAT);
			nextEpisode = LocalDateTime.parse(nextEpisodeData.jsonPath().getString(RESPONSE_EPISODE_DATE_FIELD),
					formatter);
	...
	public Show getSingleShow(String slug) {
		String showUrl = TRAKT_API_URL + slug + "?extended=full";
		Response showData = RestAssured.given().header(HEADER_CONTENT_TYPE_KEY, HEADER_CONTENT_TYPE_VALUE)
				.header(HEADER_TRAKT_API_VERSION_KEY, HEADER_TRAKT_API_VERSION_VALUE)
				.header(HEADER_TRAKT_API_KEY_KEY, HEADER_TRAKT_API_KEY_VALUE).when().get(showUrl);
	...
	public void updateDatabaseWithTopShows() {
		...
		ArrayList<Show> showsToAddToMongo = new ArrayList<>();
		...
		for (Show show : showsToAddToMongo) {
			logger.info(count + "\t\t" + show.getTitle() + " (" + show.getYearLatest() + ")");
			count++;
		}
	...
	private JSONObject buildJson(Show show) {
		JSONObject showJson = null;

		showJson = new JSONObject().put(RESPONSE_SHOW_TITLE_FIELD, show.getTitle()).put("slug", show.getIdSlug())
				.put(RESPONSE_SHOW_TRAKT_STATUS_FIELD, show.getStatus()).put("yearBegin", show.getYearBegin())
				.put("yearLatest", show.getYearLatest()).put("currentSeason", show.getCurrentSeason());
		if (null != show.getNextEpDate()) {
			showJson.put("nextEpisodeDate", show.getNextEpDate().toString()).put("nextEpisodeNum",
					show.getNextEpNumber());
		} else {
			showJson.put("nextEpisodeDate", "").put("nextEpisodeNum", "");
		}

		if (null != show.getLatestEpDate()) {
			showJson.put("latestEpisodeDate", show.getLatestEpDate().toString()).put("latestEpisodeSeasonNum",
					show.getLatestEpsSeasonNumber());
		} else {
			showJson.put("latestEpisodeDate", "").put("latestEpisodeSeasonNum", "");
		}

		showJson.put("idTrakt", show.getIdTrakt()).put("statusTrakt", show.getStatusTrakt());
	...
	public Show getNextEpisodeProper(String slugId) {
		String nextEpisodeUrl = TRAKT_API_URL + slugId + "/next_episode?extended=full";
		Response nextEpisodeData = RestAssured.given().header(HEADER_CONTENT_TYPE_KEY, HEADER_CONTENT_TYPE_VALUE)
				.header(HEADER_TRAKT_API_VERSION_KEY, HEADER_TRAKT_API_VERSION_VALUE)
				.header(HEADER_TRAKT_API_KEY_KEY, HEADER_TRAKT_API_KEY_VALUE).when().get(nextEpisodeUrl);
		...
		if (nextEpisodeData.statusCode() == 200) {
			...
			logger.info(MSG_RETRYING);
			getNextEpisodeProper(slugId); // had previously assigned to a show.
											// is this the cause of an issue
											// regarding daily update?
		}
